...
class Schema(ABC):
    ...
    """
    # 解析json 的type，返回第一个非空值，大模型接口经常返回包含 null 的联合类型，比如 ['string', 'null'] 表示可为空的字符串。
    @staticmethod
...
# tool结果是str的子类，只是新加了is_error的属性
...
class Tool(ABC):
    ...
    config_key: str = "" # 工具的类型，file相关的就是“file”
    _plugin_discoverable: bool = True # 可以被插件发现，再loader.py discover(...) 里会用到
    _scopes: set[str] = {"core"} # 该工具运行的场景，可以是多个，如{core、subagent}
...
# ---------- 宽松的类型纠偏，比如会把 “10” 转换成 int 10
    ...
    # validate_json_schema_value 严格检查参数
